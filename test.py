...
try:
	if (choose2[text] != 'None'): # если есть в списке
		...
		if (pyautogui.pixelMatchesColor(1782, 971, (255, 193, 193), tolerance=2)): # проверка на ошибки 
			if (pyautogui.pixelMatchesColor(586, 920, (255, 255, 255), tolerance=2)):
				pyautogui.moveTo(606, 901)
				pyautogui.dragTo(856, 903, duration=0.5)  # drag mouse to XY
				pyautogui.hotkey('ctrl', 'c')
				time.sleep(0.2)
				...
				with open('choose2.txt','w', encoding='UTF-8') as out:
					for key,val in choose2.items():
						out.write('{}:{}\n'.format(key,val))
			if (pyautogui.pixelMatchesColor(592, 875, (255, 255, 255), tolerance=2)):
